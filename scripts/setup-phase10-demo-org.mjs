#!/usr/bin/env node
/** Create Teller Phase 10 Demo org (never HFAC). Does not mutate Phase 5–9 demo orgs. */
...
const DEMO_ORG_NAME = "Teller Phase 10 Demo";
const FOREIGN_ORG_NAME = "Teller Phase 10 Foreign Test";
...
const BASE_ACCOUNT_SEEDS = [
  ...
  { code: "5000", name: "COGS", type: "cogs", subtype: "" },
...
async function seedSettings(supabase, organizationId) {
  ...
  await supabase.from("teller_industry_settings").upsert({
    organization_id: organizationId,
    modules,
    labels: {},
    answers: { fiscalYearStart: 7, basis: "accrual" },
  });
}
...
async function main() {
  ...
  const envKeys = {
    TELLER_PHASE10_DEMO_ORG_ID: demoOrgId,
    TELLER_PHASE10_FOREIGN_ORG_ID: foreignOrgId,
  };
