#!/usr/bin/env node
/** Apply migration 019 only, transactionally, on controlled production database. */
...
const MIGRATION_FILE = "019_phase5_banking_operations.sql";
  ...
  const { rows } = await client.query(`
    select exists (
      ...
      where n.nspname = 'public'
        and p.proname = 'teller_import_bank_transactions'
    ) as applied
  `);
...
async function main() {
  ...
  try {
    ...
    console.log("Applying migration 019 in transaction …");
