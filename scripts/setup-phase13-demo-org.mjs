#!/usr/bin/env node
/** Create Teller Phase 13 Demo org (never HFAC, never other phase demo orgs). */
...
const DEMO_ORG_NAME = "Teller Phase 13 Demo";
const FOREIGN_ORG_NAME = "Teller Phase 13 Foreign Test";

const ACCOUNTS = [
  { code: "1000", name: "Cash", type: "asset", subtype: "bank" },
  { code: "1200", name: "Inventory Asset", type: "asset", subtype: "inventory" },
  { code: "1210", name: "Inventory in Transit", type: "asset", subtype: "inventory" },
  { code: "2000", name: "Accounts Payable", type: "liability", subtype: "payable" },
  { code: "2100", name: "Goods Received Not Invoiced", type: "liability", subtype: "grni" },
  ...
  { code: "5205", name: "Purchase Price Variance", type: "expense", subtype: "ppv" },
  { code: "5100", name: "Inventory Shrinkage", type: "expense", subtype: "inventory_adjustment" },
  { code: "5110", name: "Inventory Adjustment Gain", type: "revenue", subtype: "inventory_adjustment" },
...
function assertDistinctFromPeers(orgId) {
  ...
  for (const key of PEER_ENV_KEYS) {
    ...
    if (peer && peer === orgId) throw new Error(`${key} must not equal Phase 13 demo org`);
  }
}
...
function patchEnvFile(orgId, foreignOrgId) {
  ...
  for (const [key, value] of [
    ["TELLER_PHASE13_DEMO_ORG_ID", orgId],
    ["TELLER_PHASE13_FOREIGN_ORG_ID", foreignOrgId],
  ]) {
