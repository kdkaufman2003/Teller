#!/usr/bin/env node
/** Hotfix reversal allocation RPC on controlled production (not a full migration). */
import pg from "pg";
...
const PATCH = resolve(process.cwd(), "supabase/patches/031-grni-reversal-allocation-fix.sql");
...
async function main() {
  ...
  try {
    ...
    console.log(JSON.stringify({ ok: true, patched: "031-grni-reversal-allocation-fix.sql" }, null, 2));
  } finally {
    await client.end();
  }
}
