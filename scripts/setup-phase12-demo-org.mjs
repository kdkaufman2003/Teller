#!/usr/bin/env node
/** Create Teller Phase 12 Demo org (never HFAC, never other phase demo orgs). */
...
const DEMO_ORG_NAME = "Teller Phase 12 Demo";
const FOREIGN_ORG_NAME = "Teller Phase 12 Foreign Test";

const ACCOUNTS = [
  { code: "1000", name: "Cash", type: "asset", subtype: "bank" },
  { code: "2000", name: "Accounts Payable", type: "liability", subtype: "payable" },
  { code: "2410", name: "Payroll Clearing", type: "liability", subtype: "payroll_clearing" },
  { code: "2420", name: "Federal Tax Payable", type: "liability", subtype: "payroll_tax_payable" },
  { code: "2421", name: "State Tax Payable", type: "liability", subtype: "payroll_tax_payable" },
  { code: "2422", name: "FICA Payable", type: "liability", subtype: "payroll_tax_payable" },
  { code: "2423", name: "Other Payroll Tax Payable", type: "liability", subtype: "payroll_tax_payable" },
  { code: "2430", name: "Benefits Payable", type: "liability", subtype: "payroll_payable" },
  { code: "2435", name: "Retirement Payable", type: "liability", subtype: "payroll_payable" },
  { code: "4000", name: "Revenue", type: "revenue", subtype: "" },
  { code: "6000", name: "Payroll Expense", type: "expense", subtype: "payroll" },
  { code: "6100", name: "Employer Payroll Tax Expense", type: "expense", subtype: "payroll_tax" },
  { code: "5200", name: "Direct Labor", type: "cogs", subtype: "labor" },
];
...
function assertDistinctFromPeers(orgId) {
  ...
  for (const key of PEER_ENV_KEYS) {
    ...
    if (peer && peer === orgId) throw new Error(`${key} must not equal Phase 12 demo org`);
  }
}
...
function patchEnvFile(orgId, foreignOrgId) {
  ...
  for (const [key, value] of [
    ["TELLER_PHASE12_DEMO_ORG_ID", orgId],
    ["TELLER_PHASE12_FOREIGN_ORG_ID", foreignOrgId],
  ]) {
