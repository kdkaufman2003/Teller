#!/usr/bin/env node
/** Pre/post Phase 11.1 controlled production snapshot. */
...
async function main() {
  ...
  const label = process.argv[2] === "post" ? "post-phase11-1" : "pre-phase11-1";
  ...
  const snapshot = {
    ...
    hfac: {
      ...
      phase11_1_scheduler_runs: await countRows(supabase, "teller_scheduler_runs"),
    },
