#!/usr/bin/env node
/** Create Teller Phase 11 Demo org env entries (never HFAC). Requires migration 027 after manual apply. */
...
const DEMO_ORG_NAME = "Teller Phase 11 Demo";
...
const ACCOUNTS = [
  ...
  { code: "6100", name: "Insurance Expense", type: "expense", subtype: "" },
  { code: "6200", name: "Utilities Expense", type: "expense", subtype: "" },
];
...
function patchEnv(orgId) {
  ...
  const line = `TELLER_PHASE11_DEMO_ORG_ID=${orgId}`;
  if (/TELLER_PHASE11_DEMO_ORG_ID=/.test(content)) {
    content = content.replace(/TELLER_PHASE11_DEMO_ORG_ID=.*/g, line);
  } else {
...
async function main() {
  ...
  patchEnv(orgId);
  console.log(JSON.stringify({ ok: true, organizationId: orgId, organizationName: DEMO_ORG_NAME }, null, 2));
}
