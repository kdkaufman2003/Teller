#!/usr/bin/env node
/** Create Teller Phase 14 Demo org (never HFAC, never other phase demo orgs). */
...
const DEMO_ORG_NAME = "Teller Phase 14 Demo";
const FOREIGN_ORG_NAME = "Teller Phase 14 Foreign Test";
...
function assertDistinctFromPeers(orgId) {
  ...
  for (const key of PEER_ENV_KEYS) {
    ...
    if (peer && peer === orgId) throw new Error(`${key} must not equal Phase 14 demo org`);
  }
}
...
function patchEnvFile(orgId, foreignOrgId) {
  ...
  for (const [key, value] of [
    ["TELLER_PHASE14_DEMO_ORG_ID", orgId],
    ["TELLER_PHASE14_FOREIGN_ORG_ID", foreignOrgId],
  ]) {
