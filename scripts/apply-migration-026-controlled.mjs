#!/usr/bin/env node
/** Apply migration 026 only, transactionally, on controlled production database. */
...
const MIGRATION_FILE = "026_phase10_financial_reporting.sql";
  ...
  const { rows } = await client.query(`
    select exists (
      ...
      where n.nspname = 'public'
        and p.proname = 'teller_gl_account_totals'
    ) as applied
  `);
...
async function main() {
  ...
  try {
    ...
    console.log("Applying migration 026 in transaction …");
