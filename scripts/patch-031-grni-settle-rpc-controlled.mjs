#!/usr/bin/env node
/** Hotfix GRNI settle RPC on controlled production (not a full migration). */
import pg from "pg";
...
const PATCH = resolve(process.cwd(), "supabase/patches/031-grni-settle-rpc-fix.sql");
...
async function main() {
  ...
  try {
    ...
    console.log(JSON.stringify({ ok: true, patched: "031-grni-settle-rpc-fix.sql" }, null, 2));
  } finally {
    await client.end();
  }
}
