#!/usr/bin/env node
/**
 * Apply migration 024 transactionally on controlled production.
 * Requires: TELLER_CONTROLLED_PROD_TEST=1, SUPABASE_DB_URL in .env.controlled-prod.local
 *
 * Usage: npm run migrate:phase8:controlled
 */
...
const MIGRATION = "024_phase8_fixed_assets.sql";

async function migration024Applied(client) {
  const { rows } = await client.query(`
    select exists (
      select 1 from information_schema.tables
      where table_schema = 'public'
        and table_name = 'teller_fixed_assets'
    ) as applied
  `);
...
async function main() {
  ...
  try {
    if (await migration024Applied(client)) {
      console.log(
        JSON.stringify(
          {
            ok: true,
