    ...
    try {
      ...
      const response = await fetch("/api/credit-memos", {
        ...
        body: JSON.stringify({
          partyId,
          invoiceId,
          applyToInvoiceId: applyNow ? invoiceId : undefined,
          reason,
          issueDate: todayISO(),
          lines: [
            {
              description: reason || "Credit memo",
              quantity: 1,
      ...
      if (!response.ok) throw new Error(payload.error || "Could not create credit memo");
      setOpen(false);
      router.refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Could not create credit memo");
    } finally {
      setPending(false);
    }
  }
    ...
    <div className="space-y-3">
      {!open ? (
        ...
        <form className="card p-4 space-y-3" onSubmit={(event) => void submit(event)}>
          <div>
            <p className="font-medium">Credit memo</p>
            <p className="text-sm text-muted">
              Invoice total stays {invoiceTotal.toFixed(2)} — credit reduces AR separately.
            </p>
          </div>
