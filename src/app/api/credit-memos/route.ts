...
import { postCreditMemoOpen, applyDocumentCredit, voidCreditDocument } from "@/lib/accounting/credits";
...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .select("id, number, status, total, issue_date, memo, party_id, applies_to_document_id")
...
export async function POST(request: Request) {
  ...
  if (!body.partyId) return jsonError("Customer is required");
  ...
  const appliesTo = body.invoiceId || body.applyToInvoiceId || null;
  ...
  if (appliesTo) metadata.source_invoice_id = appliesTo;
  ...
  if (error || !doc) return jsonError(error?.message || "Could not create credit memo", 500);
  ...
  if (body.post !== false) {
    ...
    if (body.applyToInvoiceId) {
      await applyDocumentCredit(supabase, {
        ...
        targetDocumentId: body.applyToInvoiceId,
        amount: total,
        actorId: session.userId,
      });
    }
